# 数据结构与算法
## 知识点梳理
* 基本概念，算法效率，时间复杂度，空间复杂度，复杂度分析
* 基本的数据结构 ：数组、链表、栈、队列、堆、散列表、二叉树、跳表、图、trie树
* 基本的算法：递归、排序、二分查找、搜索、hash算法、贪心算法、分治算法、回溯算法、动态规划、字符串匹配算法
* 熟悉的查找算法有哪些（二分查找）
* 熟悉的排序算法有哪些（冒泡排序）
* 一些基本数据结构的实现
* 
...
## 复杂度分析
复杂度分析是衡量算法性能的标准，通过复杂度分析可以估算算法在不同规模下的时间性能和空间性能

### 时间复杂度
表示算法执行时间与数据规模之间的关系
常见的时间复杂度 O(1)、O(logn) 、O(n) 、O(nlogn)、 O(n2)
时间复杂度只需要关心最大的循环代码数，对于低阶循环代码可以直接忽略
...
元素在数组中索引位置计算：(n - 1) & hash，常用的查找位的方式是取模的方式：hash%n
当n为2的n次方时，上面两种方式的计算结果一致，使用位运算比较高效
