...
## Kafka 知识点
* kafka发送消息的两种模式
* kafka如何保证消息的可靠性（通过ack应答机制，生产者消费者broker三个方面考虑）
* kafka如何保证消息的有序性（分区内有序）
...
* 异步：发送一批数据给kafka，只提供一个回调
  生产者使用带有回调方法的send，保证发送消息丢失时能正常的进行重试处理
...
## kafka集群的重要配置
broker端参数（静态参数，需要重启kafka才能生效）
存储信息配置
log.dirs  : kafka数据存储路径
log.dir

zookeeper配置
zookeeper.connect  ：zk连接信息
