...
drop uniindex index_gender on student2; --删除
...
select * from student where name = 'lisi' and card_id = '1002032'; 走索引，使用完整的索引列进行匹配
```
...
> 使用时需要修改全文索引最小修改长度 查看字段设置show variables like '%ft%'；
...
create fulltext index index_content on article(content);--插入数据后创建
...
b-tree（以最大深度为3为例，一个节点最多存储两个数据值）
> 一个节点最多存储两个数据，
> 添加数据时，如果被添加的节点存储数据量大于3，节点进行分裂，则将中间的值向上提升为父节点，两边进行分裂
...
b+tree
> 构建树的过程与b-tree一致
> 最底层的叶子节点使用双向链表进行链接，存储了索引上的所有数据（方便范围查找）
> 叶子结点在磁盘上存放在连续的空间上，进一步提高查找速度

在MyISAM中，叶子结点除了存储索引列的值以外，还存储了真实数据存放的地址，查找时先查找到地址，再通过地址取数据
...
> 首先应该考虑where和order by 设计的列上简历索引

## 11.mysql存储引擎
> 数据存储的底层软件组织
> 不同存储引擎提供不同的存储方式、索引技巧和锁定水平够功能呢
...
> type： 表连接类型 查询效率 system>const>ef_ref>ref>range>index>all
   * all 全表扫描
   * null 不访问任何表任何索引直接返回结果 ```sql select now()```
   * system 系统表，少量数据，不需要进行磁盘io，msyql5.7以上的版本访问系统表是all
   * const 查询条件是主键索引或唯一索引，且判断条件是常量
   * ref 查询条件是非唯一索引 前表的每一行对应后表只有一行被扫描，join查询，且命中的左表普通索引（普通索引列），且用等值比较 (如果不是一对一的关系，则是all)
   * eq_ref 前表的每一行对应后表只有一行被扫描，join查询，且命中的左表主键索引（主键列），且用等值比较 (如果不是一对一的关系，则是all)
   * range 范围查询
   * index 扫描索引列的全部数据
...
> extra 执行情况的说明和描述 
   * （Using where）不走索引
   * using filesort 说明mysql会对数据使用一个外部的索引排序，而不是按照索引顺序进行读取，称为文件排序，效率低
   * using temporary 需要简历临时表暂存结果，常见于order by 和group by，效率低
...
> 以%开头的like模糊查询索引失效，当select字 段是最左前缀规则下的索引列时（不实用*号），则以%开头的like还是可以走索引的
> 如果mysql聘雇使用索引列比全表更慢，则不回使用索引列
...
> order by优化
   ...
   * 避免多个排序字段时排序方式不同，都用升序asc或降序asc
   * 多个排序字段尽量和组合索引字段顺序一致
   * 设置max_length_for_sort+data的大小和sort_buffer_size的大小，优先使用一次扫描
...
## 16. sql注入
> 不使用prepareStatement时，使用拼接的方式改变原来sql的逻辑，导致与原来sql不同的意图 ```sql where username='aaa' and password ='bbb' or 1=1```
> 使用PrepareStatement,使用占位符的形式进行参数的填充
