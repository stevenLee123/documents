...
> 性能： synchronized适用于少量同步的情况，性能开销较大；Lock适用于大量同步阶段，
...
当有线程上下文切换时，需要保存系统当前的状态，并恢复另一个线程的状态，这个操作由程序计数器来完成，记住下一条jvm指令的执行地址，时线程私有的
**频繁的上下文切换会影响性能**
...
> 如果cas交换失败，有两种情况
  ...
  * 如果是自己执行了synchronized锁重入，那么再添加一条lock record作为冲入的计数
