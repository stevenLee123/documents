...
* JMM内存模型
* happens before规则规定的是什么，有哪些具体的规则
* CAS、lock、synchronize的区别，应用场景
* volatile的实现原理及使用场景
* synchronized的实现原理、锁膨胀过程
* 线程之间通信的方式（wait/notify、join（底层实现使用wait/notify））、如何控制线程的执行顺序（使用wait/notify、使用ReentrantLock、使用park/unpark）
* Future的实现原理（通过wait/notify来实现有返回结果的线程执行）
* wait/notify使用时为什么需要获取对象锁
* 线程间协作工具CountDownLatch、CyclicBarrier、Semaphore的用法，协同线程同时执行完成
* 线程池的使用，线程池的参数含义，线程池中各个参数在各种场景中的配置，IO密集型、CPU密集型场景下线程数量的选择
* 线程池中的阻塞队列、拒绝策略有哪些，在实际中你都用过哪些
* java.concurrent包中提供的并发工具有哪些，在实际工作中各有什么用途（并发工具，锁，信号量等，并发容器、原子类、线程池、异步编程工具future、CompletableFuture）
* 什么是AQS（AbstractQuenedSynchronizer） 并发编程的同步器框架
* 异步编程CompletableFuture怎么使用
## JMM java内存模型与内存屏障 Java memory model
JMM中包含的内容：
* 主内存，所有线程共享的内存区域
* 线程工作内存：每个线程都有自己的工作内存，用于存储变量的副本，线程对变量的操作都是在工作内存中，而不是直接操作主内存，线程之间不能直接操作对方的工作内存中的变量，只能通过主内存来的读取写入来影响其他线程工作内存中的变量
* 内存屏障 用于控制线程之间的内存可见性和指令重排序，分为读屏障和写屏障
* happends before 规定线程之间的操作顺序，前一个操作的结果对后一个操作的是可见的，表达的是多个线程之间操作内存的可见性
...
<init>是实例构造器，是使用new方法是执行的构造器方法，<cinit>是类构造器方法，也就是jvm进行类加载-链接-初始化中初始化阶段执行的
...
乐观锁 -- cas
悲观锁 -- Lock接口的实现类
偏向锁 -- synchronized 在只有一个线程对对象锁进行访问时使用的是偏向锁，将线程id写入markword中
轻量级锁 -- 通过cas实现，通过cas自旋的方式尝试操作对象头部的markword
重量级锁 -- 通过 monitorEnter/monitorExit来实现，当使用重量级锁时，会导致线程阻塞
CAS自旋锁
...
* 在一段代码块内，如果存在多个线程对共享资源的读写操作，这段代码称为`临界区`
...
使用wait 时必须是当前线程已经获得对象的锁，wait实际上是主动释放对象锁，如果没有获取当前对象的锁则会报错 IllegalMonitorStateException
...
### happens-before
前一个操作的结果对后续操作是可见的，约束了编译器优化的行为，允许编译器优化，但是要遵循happens-before规则

规定对共享变量的写操作对其他线程读操作可见，是可见性与有序性的一套规则
* 程序可见性规则：一个线程中，按照程序顺序，前面的操作happens-before 于后续操作
* volatile 变量规则，一个对volatile变量的写操作，happens-before于后续对volatile变量的读操作
* 传递性： 如果操作A happens-before B，B happens-before C，则C happens-before C
* 管程中的锁规则（一种同步原语，在java中synchronized是java对管程的实现）：对一个锁的解锁happens-before 于后续对这个锁的加锁
* 线程的start规则：主线程A启动子线程B之后，子线程能看到主线程在启动子线程之前的操作
* 线程join规则：主线程A等待子线程B完成（通过join方式），当子线程B完成之后，主线程能看到子线程的操作（对共享变量的操作）

### 竞态条件
竞态条件：程序的执行结果依赖于线程的执行顺序，产生竞态条件时，结果往往不正确。
```java
public class TestAdd2 {
    ...
    public static void main(String[] args) throws InterruptedException {
        TestAdd2 testAdd = new TestAdd2();
        testAdd.add();
        System.out.println(testAdd.getI());
    }
...
> 可实现简单的限流，限制访问连接数，实现数据库连接池的获取（不允许多于连接数对象个数的线程来访问连接池）
...
### CyclicBarrier
循环栅栏，(在线程任务执行完成之后可以统一执行其他任务，且barrier计数回到初始值可以复用)也是用来线程之间的同步协作，构造时设置计数个数，每个线程执行到某个需要同步的时刻调用await（）方法进行等待，当线程数满足计数个数时，继续执行
```java
@Slf4j
...
### CompletableFuture 实现复杂的异步编程
解决线程之间的聚合关系，如AND聚合或OR聚合
烧水泡茶案例
任务1:洗茶壶、烧开水
任务2:洗茶杯拿茶叶
任务3:泡茶
第三个任务需要等待前两个任务完成后才能执行
```java
//任务1：洗水壶->烧开水
...
上茶:龙井
```
### CompletionService 批量执行并行任务
将执行结果放入到阻塞队列中，实现线程执行结果的顺序返回结果
需要批量提交异步任务的时候建议使用 CompletionService
```java
// 创建线程池
...
**ThreadLocal 的实现**
每个Thread维护一个ThreadLocalMap，这个map的key是ThreadLocal本身，value是真正要存储的值，
具体过程：
...
2. 提高单机并发能力
3. 提高整体并发能力
**我们并不用去追求严格意义上的高并发经验，因为很多时候都没有这样的机会给到你，对于高并发的处理经验主要体现在日常的积累以及对自己的严格要求上。**
### 高并发的设计经验：
1. 系统的设计优化
系统无状态才能不断的进行横向扩展，才能支撑流量的变化，在业务层的高并发大多需要不断的扩容来支持。
在扩容的同时对系统进行优化，主要是对下游依赖和底层数据库层的优化，解决方案大多是常见的缓存分层、读写分离、分库分表、异步优化等。
引入这些技术方案时会带来副作用，需要我我们采取技术手段来解决和屏蔽这些副作用
2. 业务逻辑的优化
即便是在流量不大的系统中，我们也要严格要求自己，让业务跑的更快。
对于单次请求，耗时从100ms优化到20ms，单机能力从100qps提升到500qps，同时对cpu等资源的消耗不明显上涨；优化单词请求对缓存和db的放大系数降低，减少对缓存和db的压力测试。
在这个过程中积累大量的实战经验，完成搞笑且具备高扩展性的业务开发
3. 压力测试验证
在业务体量没有高并发的情况下，自己任务创造流量来验证。
对测试环境进行全链路的压力测试改造

无状态服务：对于单次请求的处理，不依赖于其他请求，处理一次请求所需的全部信息，要么包含在这个请求里，要么可以从外部获取到（如数据库），服务器本身不存储任何信息
有状态服务：自身保存一些数据，先后的请求是有关联的

### 如何展示自己的高并发经验
1. 如何优化单请求耗时： 阐述如何改造系统架构，可能涉及到jvm的参数优化、各种依赖如何选型、这个过程中积累了那些选型和优化的经验
2. 如何提高单机并发能力：阐述在业务中单机并发能力受限于那些关键点，主要的挑战是单机的限制条件及单机资源的竞争，在改造中是如何设计克服的，这个过程中积累了哪些设计原则
3. 如何提高整体的并发能力：在集群环境下提升整体并发能力，处理跨机器的资源竞争、负载均衡、数据一致性问题，体现整体的业务架构设计能力
...
考虑系统的扩展能力，将服务设计成无状态的
保证可以用性，对服务接口进行超时设置，防止线程阻塞，在慢请求上造成系统雪崩

高并发微观目标：
1. 降低系统的平均响应时间
2. 提升系统吞吐量
3. 系统的可用性指标，保证3个9或4个9
4. 可扩展性指标：在面对突发情况，不可能短时间内改造架构，最快的方式就是增加机器线性提高系统的处理能力

高并发实践：
1. 纵向扩展
* 提升单机硬件性能，通过增加内存、cpu核心数、存储容量、对磁盘进行升级（SSD）等方式提高系统并发能力
* 提升单机软件性能，考虑优化数据库查询、架设缓存、使用并发或异步的方式增加吞吐量
  
2. 横向扩展
通过集群部署的方式进一步提高并发处理能力：
* 做好分层架构，高并发系统业务复杂，做好分层处理可以简化复杂问题，更容易做到横向扩展
  * 引入CDN做动静分离，使用nginx做反响代理，web层使用统一的api网管、业务层安垂直业务做微服务化，业务层与数据库之间做好多级缓存，数据库层做读写分离
* 各个层进行水平扩展，进行无状态的水平扩容、有状态的分片路由

具体实践方案：
1. 集群部署，通过负载均衡减轻单机压力
2. 多级缓存，包括静态数据使用CDN，本地缓存、分布式缓存等，对于缓存场景中的热点key、缓存穿透、缓存并发、数据一致性问题进行处理
3. 数据库层面优化，进行sql优化、索引优化、使用es等搜索引擎解决复杂查询问题
4. 考虑NoSQL数据库，如redis、HBase等
5. 异步化，将次要流程通过多线程、MQ、延时任务进行异步处理
6. 限流，先考虑业务场景是否需要限流（如秒杀业务允许限流），包括前段限流，Nginx接入层限流、服务端限流
7. 对流量进行削峰、通过MQ承接流量
8. 并发处理，通过多线程将穿行逻辑并行化
9. 缓存预热，通过异步任务提前预热数据将数据缓存到本地或分布式缓存中
10. 减少IO次数，数据库和缓存的批量读写、RPC的批量接口支持、通过冗余数据的方式去掉RPC
11. 程序逻辑优化
12. 根据是CPU密集型还是IO密集型设置线程池核心参数
13. JVM优化，包括新生代老年代大小、垃圾回收器选择
14. 锁选择，度多些少的场景使用乐观锁、考虑使用分段锁的方式减少锁冲突
